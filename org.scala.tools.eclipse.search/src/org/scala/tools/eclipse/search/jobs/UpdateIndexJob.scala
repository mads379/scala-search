...
class UpdateIndexJob
  extends WorkspaceJob("Update the index")
  ...
  def runInWorkspace(monitor: IProgressMonitor): IStatus = {
    ...
    if (SemanticSearchPlugin.isInitialIndexRunning) {
      // If the initial indexing job isn't done we don't want to update
      // the index.
      logger.debug("Initial workspace index isn't done yet. Waiting.")
      schedule(SemanticSearchPlugin.updateInterval)
    ...
    while( it.hasNext && !monitor.isCanceled()) {
      ...
      resource.getType match {
        case IResource.FILE  => SemanticSearchPlugin.indexer.indexFile(resource.asInstanceOf[IFile])
        case _ =>
      }
    ...
    def visit(delta: IResourceDelta): Boolean = {
      ...
      resource.getType match {
        case IResource.FILE => {
          ...
          if (SemanticSearchPlugin.isIndexable(file)) {
