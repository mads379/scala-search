 ...
 *
 * Given that we're implementing en Eclipse interface here we don't have much type-safty.
...
class TypeHierarchyTreeContentProvider(
  ...
  /*  This is the base type for the `noes` in the tree. That is, instances
   ...
   * when drawing the tree and by the Job started by `findInBackground`
   * when finding sub types; multiple Jobs could be running at the
   * same time if the user expands nodes quickly.
   */
  private val cache = new ConcurrentHashMap[InputType, Seq[ElementType]]

  private var input: InputType = _
  ...
  override def inputChanged(viewer: Viewer, oldInput: Object, newInput: Object) {
    cache.clear()
    for {
      inElem <- Option(newInput) onEmpty
        logger.debug("inputChanged(), new input was null")
      newEntity <- inElem.asInstanceOfOpt[InputType] onEmpty
        logger.debug(s"Wrong element type, found ${inElem.toString}")
    } {
      input = newEntity
      viewer.refresh()
    }
  ...
  override def getElements(inputElement: Object): Array[Object] = {
    (for {
      element <- inputElement.asInstanceOfOpt[InputType] onEmpty
        logger.debug(s"getElements() got unexpected type $inputElement")
    } yield {
  ...
  override def hasChildren(element: Object): Boolean = {
    ...
    (for {
      ...
      case EvaluatedNode(hit) => Option(cache.get(hit.value)) map (!_.isEmpty) getOrElse true
      case _ => false
    }) getOrElse false
  }

  // Get the cached subtypes or start a jobs that finds them.
  private def getChildrenOf(entity: ElementType): Seq[ElementType] = {
    entity match {
      case EvaluatedNode(hit) =>
        Option(cache.get(hit.value)) getOrElse {
