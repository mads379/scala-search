...
class SearchPresentationCompiler(val pc: ScalaPresentationCompiler) extends HasLogger {
  ...
  def declarationContaining(loc: Location): Option[Occurrence] = {
    ...
    loc.cu.withSourceFile { (sf,locPc) =>
      ...
      def isContainedIn(pos: Position, types: List[Tree]) = {
        val filtered = types.filter { s => s != NoType && s != NoSymbol }
        val matches  = filtered.filter { s => s.pos != NoPosition && s.pos.point == pos.point}
        !matches.isEmpty
      }
      ...
      locPc.withParseTree(sf) { parsed =>
        ...
        new ModuleDefOrClassDefLocator(pos).locateIn(parsed) match {
          case t @ ModuleDef(_, name, Template(supers, _, body)) if isContainedIn(pos, supers) => Some(matches(name, t))
